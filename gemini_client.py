    ...
    for attempt in range(max_retries):
        try:
            ...
            if finish_reason == 'SAFETY':
                return "ERROR: Respons Gemini diblokir karena alasan keamanan (SAFETY)."
            return f"ERROR: Respons tidak valid dari Gemini API untuk {ai_type}. finishReason={finish_reason}"
            ...
            error_msg = f"ERROR tak terduga dalam generate_gemini_content: {e}"
            log_message(prompt, error_msg, ai_type)
            return error_msg
...
def get_combined_ai_response(original_prompt, focus="Kode Baru", temp=0.7, history=None):
    ...
    final_decision_text = generate_gemini_content(
        ...
        "Keputusan Akhir (Final)", temperature=temp, history=history
    )
